...
namespace RabbitMQ.Util
{
   ...
   */
    ...
    public class UIntAllocator
    {
        private readonly ushort[] unsorted;
        private IntervalList Base;
        private int unsortedCount = 0;
     ...
     */
        ...
        public UIntAllocator(ushort start, ushort end)
        {
            if (start > end)
            {
                throw new ArgumentException("illegal range [" + start + ", " + end + "]");
            ...
            // Fairly arbitrary heuristic for a good size for the unsorted set.
            unsorted = new ushort[Math.Max(32, (int)Math.Sqrt(end - start))];
            Base = new IntervalList(start, end);
        }
     ...
     */
        ...
        public bool Reserve(ushort id)
        {
            ...
            while (current != null)
            {
                ...
                else
                {
                    ...
                    // The ID is in the middle of this interval.
                    // We need to split the interval into two.
                    var rest = new IntervalList(Convert.ToUInt16(id + one), current.End);
                    current.End = Convert.ToUInt16(id - one);
                    rest.Next = current.Next;
                    current.Next = rest;
                }
                return true;
        ...
        public class IntervalList
        {
            ...
            public IntervalList(ushort start, ushort end)
            {
                Start = start;
                End = end;
            }
            ...
            public static IntervalList FromArray(ushort[] xs, int length)
            {
                Array.Sort(xs, 0, length);
